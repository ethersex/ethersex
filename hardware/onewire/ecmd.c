...
/*
  ...
  ecmd_feature(onewire_convert, "1w convert", [DEVICE], Trigger temperature conversion of either DEVICE or all connected devices)
   ecmd_ifndef(TEENSY_SUPPORT)
    ecmd_feature(onewire_power, "1w power", DEVICE, Return temperature value and power supply of onewire DEVICE (provide 64-bit ID as 16-hex-digits))
    ecmd_feature(onewire_status, "1w status", DEVICE, Return power supply of onewire DEVICE (provide 64-bit ID as 16-hex-digits))
   ecmd_endif()
*/