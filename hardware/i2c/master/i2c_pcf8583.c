...
/*************************************************************************/
uint8_t i2c_pcf8583_set_addr(uint8_t Adr) {
/*************************************************************************/
  uint8_t ret = 1;

  if (!i2c_master_select(PCF8583_ADR, TW_WRITE)) { ret = 0; goto end; }
...
/*************************************************************************/
uint8_t i2c_pcf8583_start_read(void) {
/*************************************************************************/
  uint8_t ret = 1;
...
/*************************************************************************/
uint8_t i2c_pcf8583_set_byte(uint8_t Adr, uint8_t Val) {
/*************************************************************************/
  uint8_t ret = 1;

#ifdef DEBUG_I2C
  debug_printf("I2C: i2c_pcf8583_set_byte: 0x%X (%d) [0x%X (%d)] : [0x%X (%d)]\n",
    PCF8583_ADR, PCF8583_ADR, Adr, Adr, Val, Val);
#endif
  if (!i2c_pcf8583_set_addr(Adr)) { ret = 0; goto end; }
...
/*************************************************************************/
uint8_t i2c_pcf8583_get_byte(uint8_t Adr) {
/*************************************************************************/
  uint8_t ret = 0xff;
...
#endif
  if (!i2c_pcf8583_set_addr(Adr)) { ret = 0xff; goto end; }

  /* start reading */
  if (! i2c_pcf8583_start_read()) { ret = 0xff; goto end; }
...
/*************************************************************************/
uint8_t i2c_pcf8583_set_word(uint8_t Adr, uint16_t Val) {
/*************************************************************************/
  uint8_t ret = 1;

#ifdef DEBUG_I2C
  debug_printf("I2C: i2c_pcf8583_set_word: 0x%X (%d) [0x%X (%d)] : [0x%X (%d)]\n",
    PCF8583_ADR, PCF8583_ADR, Adr, Adr, Val, Val);
#endif
  if (!i2c_pcf8583_set_addr(Adr)) { ret = 0; goto end; }
...
/*************************************************************************/
uint16_t i2c_pcf8583_get_word(uint8_t Adr) {
/*************************************************************************/
  uint16_t ret = 0xffff;
  ...
  debug_printf("I2C: i2c_pcf8583_get_word: 0x%X (%d) [0x%X (%d)]\n",
    PCF8583_ADR, PCF8583_ADR, Adr, Adr);
#endif
  if (!i2c_pcf8583_set_addr(Adr)) { ret = 0xffff; goto end; }

  /* start reading */
  if (! i2c_pcf8583_start_read()) { ret = 0xffff; goto end; }
...
/*************************************************************************/
  ...
  if (!i2c_pcf8583_get_rtc(&dt)) return 0;
  d.sec     = dt.sec;
  d.min     = dt.min;
  d.hour    = dt.hour;
  d.dow     = dt.wday;
...
/*************************************************************************/
  ...
  if (!i2c_pcf8583_get_rtc(&dt)) return;
  d.sec     = dt.sec;
  d.min     = dt.min;
  d.hour    = dt.hour;
  d.dow     = dt.wday;
...
/*************************************************************************/
...
#ifdef DEBUG_I2C
  debug_printf("I2C: i2c_pcf8583_set_rtc: 0x%X (%d) : %04d-%02d-%02d (%d) %02d:%02d:%02d.%02d\n",
    PCF8583_ADR, PCF8583_ADR,
    dt->year, dt->mon, dt->day, dt->wday,
    dt->hour, dt->min, dt->sec, dt->hsec);
#endif
  if (!i2c_pcf8583_set_addr(PCF8583_CTRL_STATUS_REG)) { ret = 0; goto end; }
  ...
  if (!i2c_pcf8583_set_word(PCF8583_YEAR_REG, dt->year)) return 0;

  /* Enable counting */
  if (!i2c_pcf8583_set_byte(PCF8583_CTRL_STATUS_REG, PCF8583_MASK)) return 0;
...
/*************************************************************************/
...
#endif
  if (!i2c_pcf8583_set_addr(PCF8583_100S_REG)) { ret = 0; goto end; }

  /* start reading */
  if (! i2c_pcf8583_start_read()) { ret = 0; goto end; }
  ...
  if (YearReg == 0xffff) return 0;

  /* Increment passed years */
  YearReg += (YearCnt - (YearReg & 0x03)) & 0x03;

  /* Now update the year byte in the NVRAM */
  if (!i2c_pcf8583_set_word(PCF8583_YEAR_REG, YearReg)) return 0; 

  dt->year = YearReg;

#ifdef DEBUG_I2C
  debug_printf("I2C: i2c_pcf8583_get_rtc value: %04d-%02d-%02d (%d) %02d:%02d:%02d.%02d\n",
    dt->year, dt->mon, dt->day, dt->wday,
    dt->hour, dt->min, dt->sec, dt->hsec);
#endif
