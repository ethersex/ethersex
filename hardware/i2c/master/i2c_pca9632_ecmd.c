...
#endif
        //  i2c_pca9632_set(I2C_SLA_PCA9531 + adr, period, duty, 0x00, 0x40, 0xEF, 0x55);
        i2c_pca9632_reset();
...
parse_cmd_i2c_pca9632_set_reg(char *cmd, char *output, uint16_t len)
{
        ...
        uint8_t mode2;
        uint8_t ledstate;
        sscanf_P(cmd, PSTR("%hhx %hhx %hhx %hhx"), &adr, &mode1, &mode2, &ledstate);
        ...
        // return ECMD_FINAL_OK;
        return ECMD_FINAL(snprintf_P(output, len, PSTR("set modes ok")));
}
...
parse_cmd_i2c_pca9632_set_blink(char *cmd, char *output, uint16_t len)
{
        ...
        // return ECMD_FINAL_OK;
        return ECMD_FINAL(snprintf_P(output, len, PSTR("set grp modes ok")));
}
...
parse_cmd_i2c_pca9632_set_led(char *cmd, char *output, uint16_t len)
{
        ...
        // return ECMD_FINAL_OK;
        return ECMD_FINAL(snprintf_P(output, len, PSTR("led_x pwm ok")));
}
