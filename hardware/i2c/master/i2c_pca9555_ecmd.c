...
int16_t parse_cmd_i2c_pca9555_out(char *cmd, char *output, uint16_t len)
{
	...
	return ECMD_FINAL(snprintf_P(output, len, PSTR("i2c pca9555 out 0x%02X"), ret));
#else
...
#ifdef ECMD_MIRROR_REQUEST
	return ECMD_FINAL(snprintf_P(output, len, PSTR("i2c pca9555 in 0x%02X"), ret));
#else
	return ECMD_FINAL(snprintf_P(output, len, PSTR("0x%02X"), ret));
#endif
}
...
#ifdef ECMD_MIRROR_REQUEST
	return ECMD_FINAL(snprintf_P(output, len, PSTR("i2c pca9555 mode 0x%02X"), ret));
#else
	return ECMD_FINAL(snprintf_P(output, len, PSTR("0x%02X"), ret));
#endif
}
