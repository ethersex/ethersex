...
process_packet(void)
{
  ...
  enc28j60_next_packet_pointer =
    read_buffer_memory() | (read_buffer_memory() << 8);
  ...
  {
    write_control_register(REG_ERXRDPTL,
                           LO8(enc28j60_next_packet_pointer - 1));
    write_control_register(REG_ERXRDPTH,
                           HI8(enc28j60_next_packet_pointer - 1));
  }
