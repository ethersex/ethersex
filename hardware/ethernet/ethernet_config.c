...
#if (defined(IPV4_SUPPORT) && !defined(BOOTP_SUPPORT) && \
    !defined(DHCP_SUPPORT)) || defined(IPV6_STATIC_SUPPORT)
  uip_ipaddr_t ip;
...
#if UIP_CONF_IPV6
  if (counter == 5) {
    // Send a router solicitation every 10 seconds, as long as we only got a
    // link local address.  First time one second after boot
