...
uint8_t
read_control_register(uint8_t address)
{
  /* change to appropriate bank */
...
void
write_control_register(uint8_t address, uint8_t data)
{
...
void
write_buffer_memory(uint8_t data)
{
  /* aquire device */
  cs_low();
...
void
bit_field_modify(uint8_t address, uint8_t mask, uint8_t opcode)
{
  /* change to appropiate bank */
  if ( (address & REGISTER_ADDRESS_MASK) < KEY_REGISTERS &&
      ((address & REGISTER_BANK_MASK) >> 5) != (enc28j60_current_bank))
...
void noinline
set_read_buffer_pointer(uint16_t address)
{
...
uint16_t noinline
get_read_buffer_pointer(void)
{
  return ( read_control_register(REG_ERDPTL) |
          (read_control_register(REG_ERDPTH) << 8));
}

void noinline
set_write_buffer_pointer(uint16_t address)
{
...
uint16_t noinline
get_write_buffer_pointer(void)
{
  return ( read_control_register(REG_EWRPTL) |
          (read_control_register(REG_EWRPTH) << 8));

}

uint16_t
read_phy(uint8_t address)
{
  ...
  return ( read_control_register(REG_MIRDL) |
          (read_control_register(REG_MIRDH) << 8));

}

void
write_phy(uint8_t address, uint16_t data)
{
  /* set address */
  write_control_register(REG_MIREGADR, address);
...
init_enc28j60(void)
{
  ...
  /* enable interrupts */
  write_control_register(REG_EIE,
                         _BV(INTIE) | _BV(LINKIE) | _BV(PKTIE) | _BV(TXIE) |
                         _BV(TXERIF) | _BV(RXERIF));
...
int16_t
parse_cmd_enc_dump(char *cmd, char *output, uint16_t len)
{
  (void) cmd;
  (void) output;
  (void) len;
  ...
  debug_printf
    ("        0x%02x    0x%02x    0x%02x    0x%02x  %02x:%02x:%02x:%02x:%02x:%02x\n",
     read_control_register(REG_MACON1), read_control_register(REG_MACON2),
     read_control_register(REG_MACON3), read_control_register(REG_MACON4),
     read_control_register(REG_MAADR5), read_control_register(REG_MAADR4),
     read_control_register(REG_MAADR3), read_control_register(REG_MAADR2),
     read_control_register(REG_MAADR1), read_control_register(REG_MAADR0));

  debug_printf
    ("Rx   : ERXST  ERXND  ERXWRPT ERXRDPT ERXFCON EPKTCNT MAMXFL\n");
  debug_printf
    ("      0x%02x%02x 0x%02x%02x   0x%02x%02x  0x%02x%02x    0x%02x    0x%02x 0x%02x%02x\n",
     read_control_register(REG_ERXSTH),   read_control_register(REG_ERXSTL),
     read_control_register(REG_ERXNDH),   read_control_register(REG_ERXNDL),
     read_control_register(REG_ERXWRPTH), read_control_register(REG_ERXWRPTL),
     read_control_register(REG_ERXRDPTH), read_control_register(REG_ERXRDPTL),
     read_control_register(REG_ERXFCON),  read_control_register(REG_EPKTCNT),
     read_control_register(REG_MAMXFLH),  read_control_register(REG_MAMXFLL));

  debug_printf("Tx   : ETXST  ETXND  MACLCON1 MACLCON2 MAPHSUP\n");
  debug_printf
    ("       0x%02x%02x 0x%02x%02x   0x%02x     0x%02x     0x%02x\n",
     read_control_register(REG_ETXSTH),   read_control_register(REG_ETXSTL),
     read_control_register(REG_ETXNDH),   read_control_register(REG_ETXNDL),
     read_control_register(REG_MACLCON1), read_control_register(REG_MACLCON2),
