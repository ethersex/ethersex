...
/* prototypes */
fs_status_t noinline fs_init(fs_t *fs, df_chip_t chip); /* initialize filesystem, scan dataflash, format if no filesystem is found */
fs_status_t noinline fs_list(fs_t *fs, char *dir, char *buf, fs_index_t index); /* list files in directory, write filename to buffer, return FS_OK or FS_EOF if no more */
