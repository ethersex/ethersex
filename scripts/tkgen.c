    ...
    int menu_num, int line_num )
{
	...
	||   cfg->token == token_ipv4 || cfg->token == token_ipv6 
	||   cfg->token == token_mac)
	    return;
    }
...
void generate_writeconfig( struct kconfig * cfg )
{
    ...
    case token_ipv4:
	printf( "write_ipv4 $cfg $autocfg %s \"$%s\" $notmod",
	    vartable[cfg->nameindex].name, vartable[cfg->nameindex].name );
	if ( cfg->cond != NULL )
	    printf( " }" );
	printf( "\n" );
    ...
    case token_ipv6:
	printf( "write_ipv6 $cfg $autocfg %s \"$%s\" $notmod",
	    vartable[cfg->nameindex].name, vartable[cfg->nameindex].name );
	if ( cfg->cond != NULL )
	    printf( " }" );
	printf( "\n" );
    ...
    case token_mac:
	printf( "write_mac $cfg $autocfg %s \"$%s\" $notmod",
	    vartable[cfg->nameindex].name, vartable[cfg->nameindex].name );
	if ( cfg->cond != NULL )
	    printf( " }" );
	printf( "\n" );
