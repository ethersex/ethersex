...
vfs_eeprom_find_free_page(vfs_eeprom_inode_t suggested)
{
  ...
  while(1) {
    if (!vfs_eeprom_read_page(tmp, buf, 1)) return 0;
...
vfs_size_t
vfs_eeprom_read(struct vfs_file_handle_t *handle, void * buffer, vfs_size_t size)
{
  ...
  vfs_eeprom_inode_t pages_skip = handle->u.ee.offset / sizeof(data_page->data);
...
vfs_size_t
vfs_eeprom_write(struct vfs_file_handle_t *handle, void *data, vfs_size_t len)
{  
  unsigned char buf[SFS_PAGE_SIZE];
  struct vfs_eeprom_page_file *file_page = (struct vfs_eeprom_page_file *) buf;
  struct vfs_eeprom_page_data *data_page = (struct vfs_eeprom_page_data *) buf;
  ...
  vfs_eeprom_inode_t pages_needed = (handle->u.ee.offset + len) / sizeof(data_page->data) + 1;
  ...
  vfs_eeprom_inode_t write_offset = handle->u.ee.offset % sizeof(data_page->data);
  ...
  while (len) {
    ...
    vfs_eeprom_write_slice(write_page, 4 + write_offset, 
                    data, copy_len);
