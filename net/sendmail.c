...
mail_send (void)
{
  if (mail_get == 0)		//Falls nicht derzeit bereits eine Mail gesendet wird
	...
	{
	  for (unsigned long a = 0; a < 2000000; a++)
	    {
	      asm ("nop");
	    };

	  MAIL_DEBUG ("Mail empfang am Clientport (%i)", my_mail_cp);
	  tcp_port_open (MAIL_SERVER, LBBL_ENDIAN_INT (MAIL_PORT),
			 LBBL_ENDIAN_INT (my_mail_cp));
	  mail_send_counter = 0;
	  mail_get = 1;
	  ...
	  while ((index >= MAX_ARP_ENTRY)
	    ...
	    {
	      index =
		tcp_entry_search (MAIL_SERVER, LBBL_ENDIAN_INT (MAIL_PORT));
	      if (tmp_counter++ > 30)
		{
