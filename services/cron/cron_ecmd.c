...
int16_t parse_cmd_cron_list (char *cmd, char *output, uint16_t len)
{
	...
	if (!state->pos) return ECMD_FINAL_OK;
...
int16_t parse_cmd_cron_save (char *cmd, char *output, uint16_t len)
{
	...
	if (ret < 0) return ECMD_FINAL(snprintf_P(output, len, "write error"));

	return ECMD_FINAL(snprintf_P(output, len, PSTR("%d jobs saved"), ret));
}

uint16_t parse_cmd_cron_persistent (char *cmd, char *output, uint16_t len)
{
	...
	if (jobll == NULL) return ECMD_ERR_PARSE_ERROR;
...
#ifdef CRON_ANACRON_SUPPORT
uint16_t parse_cmd_cron_anacron (char *cmd, char *output, uint16_t len)
{
	uint8_t jobposition, state;
	uint8_t ret = sscanf_P(cmd, PSTR("%hhu %hhu"), &jobposition, &state);
...
int16_t parse_cmd_cron_add (char *cmd, char *output, uint16_t len)
{
	...
	if (sscanf_P(cmd, PSTR("%hhi %hhi %hhi %hhi %hhi %n"), &minute, &hour, &day, &month, &dayofweek, &i) != 5)
	{
		return ECMD_ERR_PARSE_ERROR;
	}

	int16_t ret = cron_jobinsert_ecmd(minute, hour, day, month, dayofweek, INFINIT_RUNNING, CRON_APPEND, cmd+i);
	if (ret < 0)
	{
