...
struct dmx_slot dmx_universes_state[DMX_STORAGE_UNIVERSES][DMX_STORAGE_SLOTS]
  = { {{DMX_UNCHANGED, DMX_SLOT_FREE}} };
...
void
set_dmx_channels(uint8_t * start, uint8_t universe, uint16_t len)
{
...
enum dmx_state
get_dmx_universe_state(uint8_t universe, int8_t slot)
{
  if (universe < DMX_STORAGE_UNIVERSES && slot < DMX_STORAGE_SLOTS &&
      slot >= 0)
