...
#define UDPSTELLA_REQUEST_PINS _BV(6)
#define UDPSTELLA_REQUEST_SETPINON _BV(5)
#define UDPSTELLA_REQUEST_SETPINOFF _BV(4)
/* or:
...
UDPSTELLA_REQUEST_PINS + 0 + 0
The answer for 4 ports will be something like this:
UDPSTELLA_ANSWER_PINS + portA + portB + portC + portD

Set the pin 1(0) on PortD(3) to on (analog with off):
UDPSTELLA_REQUEST_SETPINON + 3 + 0
...
#define UDPSTELLA_ANSWER_PINS 50
...
udpstella_net_main(void)
{
    ...
    while (len>=sizeof(struct udpstella_packet)) {
	    ...
	    else if (packet->type & UDPSTELLA_REQUEST_SETPINON) {
		vport[packet->channel].write_port(packet->channel,
			vport[packet->channel].read_port(packet->channel) | _BV(packet->value));
	    }
	    else if (packet->type & UDPSTELLA_REQUEST_SETPINOFF) {
		vport[packet->channel].write_port(packet->channel,
			vport[packet->channel].read_port(packet->channel) & ~_BV(packet->value));
	    } else {
