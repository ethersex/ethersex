...
/* Decrypt the cast5 encrypted OpenVPN packet and verify the packet id.
...
openvpn_decrypt_and_verify(void)
{
  ...
  /* initial IV. */
  unsigned char *cbc_carry_this = uip_appdata + OPENVPN_HMAC_LLH_LEN;
...
openvpn_encrypt(void)
{
  ...
  for (ptr = encrypt_start + 8;
       ptr < ((unsigned char *) uip_sappdata) + uip_slen; ptr += 8)
  {
...
openvpn_handle_udp(void)
{
  ...
  memmove(uip_buf + BASE_LLH_LEN,
          uip_buf + OPENVPN_TOTAL_LLH_LEN, uip_len - OPENVPN_HMAC_CRYPT_LEN);
...
openvpn_process_out(void)
{
  ...
  memmove(uip_buf + OPENVPN_TOTAL_LLH_LEN, uip_buf + BASE_LLH_LEN, uip_len);
