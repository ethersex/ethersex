...
/*
 ...
 ---------------------------------------------
   ...
   In order to be able to have more than one uIP stack available, we need to
   do some demultiplexing.  On the whole there are some variables shared
   between all stacks and some are separate.

   If multi-stack-support is activated every uip_ symbol is actually a pointer
   to the real variable, function, etc.  In order to achieve this, we rename
   every symbol from uip_ to uip_real_. Further stacks have alternate names.
*/
