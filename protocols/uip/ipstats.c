...
#define uip_stat (* (uip_stacks[cmd[1]].uip_stat))
  ...
  switch (cmd[2])
  {
    ...
    case IP:
      len =
        snprintf_P(output, len, PSTR("  ip:   recv %5d sent %5d drop %5d"),
                   uip_stat.ip.recv, uip_stat.ip.sent, uip_stat.ip.drop);
      break;
#ifdef ICMP_SUPPORT
    case ICMP:
      len =
        snprintf_P(output, len, PSTR("  icmp: recv %5d sent %5d drop %5d"),
    ...
    case UDP:
      len =
        snprintf_P(output, len, PSTR("  udp:  recv %5d sent %5d drop %5d"),
    ...
    case TCP:
      len =
        snprintf_P(output, len, PSTR("  tcp:  recv %5d sent %5d drop %5d"),
