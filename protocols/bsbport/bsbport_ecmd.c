...
parse_cmd_bsbport_stats(char *cmd, char *output, uint16_t len)
{
  int16_t chars = snprintf_P(output, len,
                             PSTR("%uOK %uCRC %uLUV %uLOV %uDROP %uBF %uBNF %uRT"),
                             bsbport_rx_ok,
                             bsbport_rx_crcerror,
...
int16_t parse_cmd_bsbport_list(char *cmd, char *output, uint16_t len)
{
	/*BUG prevention*/
	len=len-1;
  ...
  if (i >= BSBPORT_MESSAGE_BUFFER_LEN)
	...
	int16_t ret = 0;
	ret = snprintf_P(output, len, PSTR("%02d %02x%02x%02x%02x %.1f %.1f %.1f %d")
										 , i
...
int16_t parse_cmd_bsbport_get (char *cmd, char *output, uint16_t len)
{
	/*BUG prevention*/
	len=len-1;
	...
	debug_printf("ECMD get MSG found at: %02d ",i);
				...
				{
					ret = snprintf_P(output, len, PSTR("%.1f"), bsbport_msg_buffer.msg[i].value_temp);
				}
				else if(strcmp_P(type,PSTR("FP1"))==0)
				{
					ret = snprintf_P(output, len, PSTR("%.1f"), bsbport_msg_buffer.msg[i].value_FP1);
				}
				else if(strcmp_P(type,PSTR("FP5"))==0)
				{
					ret = snprintf_P(output, len, PSTR("%.1f"), bsbport_msg_buffer.msg[i].value_FP5);
				}
				else
...
int16_t parse_cmd_bsbport_query (char *cmd, char *output, uint16_t len)
{
	uint8_t p1=0;
	uint8_t p2=0;
	uint8_t p3=0;
	...
	uint8_t dest=0;
	int16_t ret = 0;
	ret = sscanf_P(cmd, PSTR("%hhi %hhi %hhi %hhi %hhi"), &p1, &p2, &p3, &p4, &dest);
	if (ret == 4 || ret == 5)
	{
		if(bsbport_query(p1,p2,p3,p4,dest)) return ECMD_FINAL_OK;
...
int16_t parse_cmd_bsbport_set (char *cmd, char *output, uint16_t len)
{
	float val=0;
	uint8_t p1=0;
	uint8_t p2=0;
