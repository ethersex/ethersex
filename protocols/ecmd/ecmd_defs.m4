...
ecmd_ifndef(TEENSY_SUPPORT)
  ecmd_ifdef(UIP_SUPPORT)
    ...
    ecmd_ifndef(IPV6_SUPPORT)
      ecmd_feature(show_netmask, "show netmask",,Display the network mask (IPv4 only).)
    ecmd_endif()
    ...
    ecmd_ifndef(IPV6_SUPPORT)
	  ...
	  ecmd_feature(netmask, "netmask ",IP,Set the network mask (IPv4 only).)
...
ecmd_ifdef(DNS_SUPPORT)
  ...
  ecmd_feature(show_dns_server, "show dns server",, Display the IP address of the configured DNS server.)
    ecmd_feature(dns_server, "dns server", IPADDR, Set the IP address of the DNS server to use to IPADDR.)
ecmd_endif()

ecmd_ifdef(NTP_SUPPORT)
  block(NTP)
  ecmd_feature(show_ntp_server, "show ntp server",, Display the IP address of the configured NTP server.)
  ecmd_feature(ntp_query, "ntp query",, Query the NTP server to get an NTP update.)
    ecmd_feature(ntp_server, "ntp server ", IPADDR, Set the IP address of the NTP server to use to IPADDR.)
ecmd_endif()
