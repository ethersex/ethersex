...
static aliascmd_t aliascmdlist[] PROGMEM = {
#ifdef UIP_SUPPORT
	{"i", 			"show ip"},
#endif
#ifdef RFM12_ASK_SENDER_SUPPORT
	{"off", 		"rfm12 2272 0,5,80 76 10"},
	{"on", 			"rfm12 2272 0,5,85 76 10"},
#endif
#ifdef I2C_LM75_SUPPORT
	{"temp", 		"lm75 0"},
#endif
#ifdef I2C_PCA9531_SUPPORT
	{"servo center","pca9531 0 20 80 2 ea ae ee"},
	{"servo left", 	"pca9531 0 20 80 2 e0 ae ee"},
	{"servo right",	"pca9531 0 20 80 2 f5 ae ee"},
#endif
...
#endif
		if (strcmp(cmd+1, alias.name)==0){ // +1 because the first char is the marker for alias names
			int aliaslen = strlen(alias.name);
			int newlen = strlen(alias.cmd);

			memmove(cmd + newlen, cmd + aliaslen, strlen (cmd + aliaslen) +1);
			memmove(cmd, alias.cmd, newlen + 1);
